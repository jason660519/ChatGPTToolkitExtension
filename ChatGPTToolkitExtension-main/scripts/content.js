    ...
    const StartMonitoringResponse = () => {
        ...
        if (currentLocale) {
            if (currentLocale == 'zh-TW') {
                ...
                defaultManualSubmitText.push({text: "Translate to En", value: "Please translate the above response into English.",});
                defaultManualSubmitText.push({text: "Translate to TW", value: "請將上述回應內容翻譯成臺灣常用的正體中文",});
                defaultManualSubmitText.push({text: "Explain Code in EN", value:"You are now a senior programmer. Please annotate the above code line by line using appropriate comment characters and explain it to an 18-year-old university student who has just started studying computer science. The goal is to help them understand the functionality and principles of this code.",});
                defaultManualSubmitText.push({text: "Summaize in EN", value:"Summarize the above content into 30 key points",});
                defaultManualSubmitText.push({text: "Explain Code in CH", value:"#zh-tw 你現在是資深程式設計師，請使用適當的註釋字符把上述代碼逐行用臺灣常用的正體中文註釋並講講解給18歲剛開始念電腦科學系的大學生，讓他們理解這些代碼的功能與原理。",});
                defaultManualSubmitText.push({text: "Table Creation", value:"#zh-tw 你現在表格整理大師，請把上面的專業詞彙用表格方式呈現，第一欄位:名稱(URL或wiki的超連結)，第二欄位:英文解釋(不超過30字),第三欄位:臺灣常用的正體中文解釋(不超過30字)",});
                defaultManualSubmitText.push({text: "Github Repo Table Creation", value:"#zh-tw 你現在Github分析大師，請把上面的Github Repo依Repo 的完整URL網址、名稱、功能特色、download數/月、Contrubutors貢獻者人數、License、使用的language與%、使用者人數, 以表格說明方式呈現",});
                defaultManualSubmitText.push({text: "Arxiv Analyser", value:"#zh-tw 你現在Arxiv 論文分析大師，請把上面的專業名詞依論文arxiv 的完整URL網址、論文主題、論文簡介、關鍵結論、論文使用的工具與開發語言、引用人數(Times Cited_, 以表格說明方式呈現",});
